...
expect(ws.send).toHaveBeenCalledWith(expect.stringContaining('"result"'));
```
...
chat: vi.fn().mockImplementation(() => {
  ...
  return call === 1
    ? {
        content: [{ type: 'tool_use', id: 't1', name: 'exec', input: { cmd: 'ls' } }],
...
let tmpDir: string;
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'oclaw-test-'));
});
...
## Environment Variables Teardown

When modifying `process.env` in tests, fully remove variables with `delete` instead of setting them to `undefined` (which sets their string value to `"undefined"`). Use a biome ignore comment to suppress the deletion performance warning:
